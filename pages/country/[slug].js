    ...
    const getWeather = async () => {
      try {
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=${countryData.capital}&units=metric&appid=${process.env.NEXT_PUBLIC_OPEN_WEATHER_API_KEY}`
        );
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
        ...
        <div>
            ...
            <>
              ...
              <ul className='flex flex-wrap'>
                {countryData.languages.map(l => (
                  <li
                    className='m-1 px-2 py-1 rounded shadow bg-gray-300'
                    key={l.name}>
                    {l.name}{' '}
                  </li>
                ))}
          ...
          {countryData.currencies && (
            <>
              <h4 className='py-1 text-lg leading-snug font-bold'>
                Currencies
              </h4>
              <ul className='flex flex-wrap'>
                {countryData.currencies.map(
                  c =>
                    c.name && (
                      <li className='m-1' key={c.name}>
                        {c.name}{' '}
                        {c.symbol && (
                          <span className='py-1 px-2 shadow font-bold bg-gray-300 rounded'>
                            {c.symbol}
                          </span>
                        )}
                      </li>
